...
import network.chat.ChatServer.ChatService;

import java.net.*;

public class ChatServer_nb implements Runnable {
...
//				클라이언트와 통신하고있는 소켓(객체)에 각각 write한다. 
				ChatService_nb cs = new ChatService_nb(s);
				cs.start();
				vc.add(cs);
	...
	public static void main( String [] arg ) {
		ChatServer_nb cs = new ChatServer_nb();
		new Thread(cs).start();
	}	// main ends



class ChatService_nb extends Thread {
...
//				Tokenizer로 밑에 명령어처럼 만들어진 부분을 수행하려함.
				if( st.countTokens() > 1 ) {
					...
					if( temp.equalsIgnoreCase("/name")) {
						...
						changeList();
						continue;
					}else if( temp.equalsIgnoreCase("/nbadd")){
						int tokencount=0;
						String temp1 = st.nextToken();
						while(st.hasMoreTokens()){
						String value = st.nextToken();
						answer[answerCount][tokencount] = Integer.valueOf(value);
						tokencount++;
						}
						answerCount++;
						putMessageAll(myname + "님의 야구가 등록되었습니다.");
						
						continue;
					
					}else if(temp.equalsIgnoreCase("/nb" )){
						...
						while(st.hasMoreTokens()){
							...
							for(int i=0;i<count;i++){
								if(Integer.valueOf(Sa) == answer[answerCount-1][tokencount]){
									for(int j=0;j<answer[answerCount-1].length;j++){
										if(answer[answerCount-1][tokencount] == answer[answerCount-1][j]){
											SC++;
											break;
										}else{
											BC++;
						...
						if(SC == 3){
							putMessageAll("Strike :"+SC+" Ball : "+BC);
							putMessageAll(myname + "님이 맞추셨습니다!");
							answerCount--;
						}
						putMessageAll("Strike :"+SC+" Ball : "+BC);
						continue;
	...
	void changeList(){
		...
		for (int i = 0; i < vc.size(); i++) {
			ChatService_nb cs = (ChatService_nb)vc.get(i);
			msg += cs.myname + " ";
		}	
		putMessageAll(msg);
	}
